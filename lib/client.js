...
window.__ModuleLoader__.load({ id: 'dsh-granular-settings', factory: (require) => {
  ...
  const { Input, Menu, Pill, Tooltip, IconChevronDownOutline14, IconSearchOutline16 } = require('@deepseek-ai/dsh-client-ui-primitives')
  ...
  const Row = (props) => {
    ...
    return React.createElement('div', { className: 'gsv-row' },
      React.createElement('div', { className: 'gsv-row-main' },
        React.createElement('div', { className: 'gsv-row-title' },
          reg.label,
          reg.description !== undefined
            // Description rides a hover/focus tooltip instead of printing
            // under every label; keeps long-copy plugins one-line.
            ? React.createElement(Tooltip, {
                label: reg.description, side: 'bottom', maxWidth: 320,
              }, React.createElement('span', { className: 'gsv-info', tabIndex: 0 }, '\u24D8'))
            : null)),
  ...
  module.exports = {
    ...
    apply(ctx) {
      ...
      const css = [
        ...
        '.gsv-row-title{font-size:13px;font-weight:600;display:flex;align-items:center;gap:6px}',
        '.gsv-info{flex:none;width:18px;height:18px;display:inline-flex;align-items:center;justify-content:center;font-size:13px;line-height:1;cursor:help;color:var(--dsw-alias-label-caption);border-radius:50%}',
        '.gsv-info:hover,.gsv-info:focus-visible{color:var(--dsw-alias-label-secondary);outline:none}',
