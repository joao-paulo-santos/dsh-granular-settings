 ...
 *      useSetting(ns, scope, key) -> [value, setValue, loaded] reactive
 *      state with default fallback, per-context snapshots, and optimistic
 *      writes; useKeybind(ns, scope, key, action) for keybind settings;
 *      registerTab({id,label,order}, Component) to contribute whole tabs.
...
window.__ModuleLoader__.load({ id: 'dsh-granular-settings', factory: (require) => {
  ...
  //
  // FAILURE RETRY: a wanted key whose fetch failed (404 while the host
  // still loads a restored session, a transient blip) is re-attempted with
  // capped backoff until it loads or its last want drops. Without this, a
  // fetch that fails at mount leaves the consumer on its snapshot forever:
  // nothing retries until some global event (doorbell, focus, a settings
  // visit) happens to re-ensure the key. User-visible as "opened a previous
  // session and the controls kept the previous session's values".
  const describing = new Map()    // cacheKey -> { dirty }
  const retryDelay = new Map()    // cacheKey -> next backoff ms
  const RETRY_FLOOR_MS = 250
  const RETRY_CAP_MS = 5000
  const scheduleRetry = (cacheKey) => {
    if (wanted.has(cacheKey) !== true) return      // nobody wants it anymore
    const delay = retryDelay.get(cacheKey) || RETRY_FLOOR_MS
    retryDelay.set(cacheKey, Math.min(delay * 2, RETRY_CAP_MS))
    setTimeout(() => {
      // Healed by another path (doorbell resync, focus) meanwhile? Skip.
      if (wanted.has(cacheKey) === true && cache.has(cacheKey) !== true) describe(cacheKey)
    }, delay)
  }
    ...
    return fetch(url)
      .then((r) => (r.ok ? r.json() : null)).then((body) => {
        ...
        if (body !== null && typeof body === 'object') {
          ...
          // Failed load: the retry timer owns the next attempt (a dirty
          // re-run would race it; 250ms beats an immediate stampede).
          scheduleRetry(cacheKey)
        }
      }, () => { describing.delete(cacheKey); scheduleRetry(cacheKey) })
  }
  ...
  // attach, default fallback, per-context snapshots, and keybind listening.
  // A consumer writes
  ...
  // Subscribe to one context's updates (doorbell, reconnect, focus, set
  // echo). Returns an unsubscribe function.
   ...
   *  @returns [value, setValue, loaded]. value falls back to the
  ...
  // ---- keybinds ----
  // One document-level listener walks every live binding. Combos are
  // normalized here (modifier order Ctrl+Alt+Shift+Meta, uppercased key,
  // Space for ' ') so consumers never duplicate the rules. The settings
  // page's keybind recorder shares the same normalization.
  const comboOf = (e) => {
    const parts = []
  ...
  const keybindActions = new Map()   // combo -> Set<fn>
  const onDocumentKeyDown = (e) => {
    if (isEditable(e.target) === true) return
    const actions = keybindActions.get(comboOf(e))
    if (actions !== undefined && actions.size > 0) {
      e.preventDefault()
      for (const fn of actions) { try { fn() } catch (err) {} }
    }
  }
  /** Fire `action` whenever the keybind setting `namespace/scope/key` is
   *  pressed. The action is ref-held, so inline lambdas stay fresh. No-op
   *  while the binding is empty or its context is unavailable. */
  const useKeybind = (namespace, scope, key, action) => {
    const [binding] = useSetting(namespace, scope, key)
    const actionRef = React.useRef(action)
    actionRef.current = action
    React.useEffect(() => {
      if (typeof binding !== 'string' || binding === '') return
      let set = keybindActions.get(binding)
      if (set === undefined) { set = new Set(); keybindActions.set(binding, set) }
      const fn = () => actionRef.current()
      set.add(fn)
      return () => {
        set.delete(fn)
        if (set.size === 0) keybindActions.delete(binding)
      }
    }, [binding])
  }
  ...
  // clears, Escape cancels). Normalization is the SHARED comboOf from the
  // keybind service above, so recorder and listener can never disagree.
  ...
  module.exports = {
    ...
    apply(ctx) {
      ...
      const offTracker = ctx.slots.inject('shell.overlay', () => ctx.slots.register(
        ...
        SessionTracker))
      // One document-level listener serves every useKeybind consumer.
