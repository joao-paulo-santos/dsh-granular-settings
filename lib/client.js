...
window.__ModuleLoader__.load({ id: 'dsh-granular-settings', factory: (require) => {
  ...
  const SearchBox = (props) => {
    ...
    return React.createElement('div', { className: 'gsv-searchrow', key: props.scopeKey },
      React.createElement('div', { className: 'gsv-searchhead' },
        React.createElement('h3', { className: 'gsv-h3' }, props.title),
        props.subtitle !== undefined
          ? React.createElement('p', { className: 'gsv-sub' }, props.subtitle)
          : null,
        props.count !== undefined
          ? React.createElement('span', { className: 'gsv-count' }, props.count)
          : null),
      React.createElement('div', { className: 'gsv-search' + (has ? ' gsv-search-filled' : '') },
        React.createElement('svg', {
          className: 'gsv-search-icon',
          viewBox: '0 0 16 16', width: '14', height: '14',
          'aria-hidden': 'true',
        }, React.createElement('circle', { cx: '7', cy: '7', r: '4.5', fill: 'none', stroke: 'currentColor', 'stroke-width': '1.5' }),
          React.createElement('line', { x1: '10.5', y1: '10.5', x2: '14', y2: '14', stroke: 'currentColor', 'stroke-width': '1.5', 'stroke-linecap': 'round' })),
        React.createElement('input', {
          type: 'text',
          className: 'gsv-search-input',
          value: draft,
  ...
  const Section = (props) => {
    ...
    const shown = groups.reduce((n, g) => n + g.regs.length, 0)
    const total = props.regs.length
    const count = trimmed !== ''
      ? shown + ' of ' + total
      : (total > 0 ? String(total) : undefined)
    return React.createElement('section', { className: 'gsv-section' },
      React.createElement(SearchBox, {
        scopeKey: props.title,
        onQuery: setQuery,
        title: props.title,
  ...
  module.exports = {
    ...
    apply(ctx) {
      ...
      const css = [
        ...
        '.gsv-searchrow{flex:none;display:flex;align-items:center;justify-content:space-between;gap:16px;flex-wrap:wrap;margin-bottom:4px}',
        '.gsv-searchhead{display:flex;align-items:baseline;gap:10px;min-width:0}',
        '.gsv-count{flex:none;font-size:11px;font-weight:600;letter-spacing:.04em;padding:2px 9px;border-radius:999px;color:var(--dsw-alias-label-primary);background:var(--dsw-alias-bg-layer-2);border:1px solid var(--dsw-alias-label-tertiary);opacity:.75;white-space:nowrap}',
        '.gsv-search{display:flex;align-items:center;gap:8px;flex:none;width:min(260px,100%);padding:0 10px;border-radius:999px;background:var(--dsw-alias-bg-layer-2);border:1px solid var(--dsw-alias-label-tertiary);transition:border-color .12s,box-shadow .12s}',
        '.gsv-search:focus-within{border-color:' + BLUE + ';box-shadow:0 0 0 3px rgba(59,130,246,.15)}',
        '.gsv-search-icon{flex:none;opacity:.5}',
        '.gsv-search:focus-within .gsv-search-icon{opacity:.9;color:' + BLUE + '}',
        '.gsv-search-input{flex:1;min-width:0;font:inherit;font-size:13px;padding:7px 0;color:var(--dsw-alias-label-primary);background:transparent;border:none;outline:none}',
        '.gsv-search-input::placeholder{opacity:.5}',
        '.gsv-search-clear{flex:none;display:flex;align-items:center;justify-content:center;width:20px;height:20px;padding:0;border-radius:50%;cursor:pointer;font-size:11px;line-height:1;color:var(--dsw-alias-label-primary);background:transparent;border:none;opacity:.55}',
        '.gsv-search-clear:hover{opacity:1;background:var(--dsw-alias-bg-layer-1)}',
