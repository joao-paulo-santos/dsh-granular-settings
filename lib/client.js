...
window.__ModuleLoader__.load({ id: 'dsh-granular-settings', factory: (require) => {
  ...
  const SearchBox = (props) => {
    ...
    return React.createElement('input', {
      ...
      // Empty on tab switch: each scope's search is its own question.
      key: props.scopeKey,
      onChange: (e) => { setDraft(e.target.value) },
    })
  }
  ...
  module.exports = {
    ...
    apply(ctx) {
      ...
      const css = [
        ...
        '.gsv-search{font:inherit;font-size:13.5px;padding:10px 14px 10px 36px;border-radius:0;width:100%;box-sizing:border-box;margin-top:14px;color:var(--dsw-alias-label-primary);background-color:var(--dsw-alias-bg-layer-2);background-image:url("data:image/svg+xml,' + encodeURIComponent('<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16" width="14" height="14"><circle cx="7" cy="7" r="4.5" fill="none" stroke="#9aa3ad" stroke-width="1.5"/><line x1="10.5" y1="10.5" x2="14" y2="14" stroke="#9aa3ad" stroke-width="1.5" stroke-linecap="round"/></svg>') + '");background-repeat:no-repeat;background-position:12px center;border:1px solid var(--dsw-alias-label-tertiary);transition:border-color .15s,background .15s}',
        '.gsv-search::placeholder{opacity:.75}',
        '.gsv-search:hover{background:var(--dsw-alias-bg-layer-1)}',
        '.gsv-search:focus{outline:none;border-color:' + BLUE + ';background:var(--dsw-alias-bg-layer-1);caret-color:' + BLUE + '}',
        '.gsv-search::placeholder{opacity:.45}',
        '.gsv-search::-webkit-search-cancel-button{cursor:pointer}',
