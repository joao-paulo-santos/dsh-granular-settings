...
window.__ModuleLoader__.load({ id: 'dsh-granular-settings', factory: (require) => {
  ...
  const GranularSettingsSection = (props) => {
    ...
    // The page ALWAYS opens at the top: the scroll node is reused across
    // mounts, tab switches, and the loading -> loaded transition (content
    // height grows under a stale scrollTop), which otherwise left a long
    // page scrolled to wherever it was last seen, hiding the tab strip.
    const scrollRef = React.useRef(null)
    React.useEffect(() => {
      if (scrollRef.current !== null && scrollRef.current !== undefined) {
        scrollRef.current.scrollTop = 0
      }
    }, [effectiveTab, cacheKey, d === undefined])
